    ...
    xhr.onload = function () {
      if (xhr.status >= 200 && xhr.status < 300) {
        try {
          ...
          console.log("response", xhr.message || xhr.response);
        ...
        reject(
          new Error(`錯誤狀態：${xhr.status}, ${xhr.message || xhr.response}`)
        );
      }
    };
...
function initializeFormValidation() {
  ...
  $form.validate({
    ...
    messages: {
      ...
      contact_company_phone: {
        required: "請輸入診所電話",
        companyPhone: "請輸入正確的電話號碼格式(數字或是 - + * # , ;)",
      },
